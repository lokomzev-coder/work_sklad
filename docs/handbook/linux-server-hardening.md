...
**Фактически применено на реальном сервере (Блок T, 2026-09-15)**:
разделы 1-3 и 7-9 — как описано ниже, буквально теми же командами.
Разделы 4-5 (отдельный системный пользователь + systemd unit) и 6/8
(PostgreSQL напрямую, certbot+nginx) заменены Docker Compose — см.
[deployment.md](./deployment.md) за тем, что использовано вместо них
(изоляция БД через internal docker-сеть вместо `listen_addresses`,
TLS через Caddy вместо certbot, `restart: unless-stopped` вместо
systemd unit). Дополнительно на реальном сервере: пароль sudo
пользователя сменён на сгенерированный случайный (30+ символов) —
дефолтный 4-значный PIN, с которым сервер был получен, недопустим для
машины, смотрящей в интернет.
