...
export default async function InvoicesInPage({
  ...
  const invoicesIn = await prisma.invoiceIn.findMany({
    where: { orgId: ctx.orgId },
    orderBy: { number: "desc" },
    include: { supplier: true, lineItems: true, status: true },
  });

  const canCreate = can(ctx, "invoicesIn", "create");
              ...
              invoicesIn.map((invoice) => {
                ...
                return (
                  <TableRow key={invoice.id}>
                    <TableCell>
                      <Link href={`/${org}/invoices-in/${invoice.id}`} className="font-medium hover:underline">
                        №{invoice.number}
                      </Link>
                    </TableCell>
